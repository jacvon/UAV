...
def img_generator(img_files, scale=1.0):
    for i in range(len(img_files)):
        ...
        img = get_square_img(img, Flag=False)
...
def getTransformMatrix(origImg, origImg_name, transImg, transImg_name):
    ...
    return np.array(perArr, np.float64)
...
def getOverlayImg_Distance(img1, img2, last_flag):
    img1_gray = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    _, mask1 = cv2.threshold(img1_gray, 0, 255, cv2.THRESH_BINARY)
    img2_gray = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    _, mask2 = cv2.threshold(img2_gray, 0, 255, cv2.THRESH_BINARY)
    ...
    contours_overlay = findMaxAreaContour(contours_overlay)
    assert len(contours_overlay) == 1  # 确保只有一个重叠区域

    # ** ** ** ** ** 利用重叠区域面积比来判断是否需要跳过当前图像的拼接 ** ** ** ** **
    # 重叠区域过大的拼接，不仅意义不大，反而影响拼接效果
    if not last_flag:  # 不允许跳过最后一张待拼接图
        area_threshold = 0.6
...
# 单方向上填充图像外侧黑边
@jit(nopython=True)
def overlapImgExtend(img, direction):
    # 目前放弃使用, 注意*本函数会直接操作输入图像*
    if direction == "LEFT_RIGHT" or direction == "RIGHT_LEFT":
        for x in range(img.shape[1]):
            ...
            for y in range(img.shape[0] - 1, -1, -1):
                if img[y, x].all():
                    ...
                    break
    elif direction == "UP_DOWN" or direction == "DOWN_UP":
...
def find_seam(img_segments, direction, former_direction):
    # 自适应缩放待处理图像，减少处理开销
    ...
    # TODO: 拼接拐角处可以考虑在对角线起始点生成缝合线，解决拐角拼接缺陷 (已完成)
    @jit(nopython=True)
    def search_seam_pts(energy_map, backtrack, direction, former_direction, row, col):
        seam_pts = []
        if direction == "LEFT_RIGHT" or direction == "RIGHT_LEFT":
            if former_direction == "UP_DOWN" or former_direction == "DOWN_UP":
                if former_direction == "UP_DOWN" and direction == "LEFT_RIGHT":
                    j = col - 1
                    for i in range(0, row):
                        seam_pts.append([j, i])
                        j = backtrack[i, j]
                elif former_direction == "UP_DOWN" and direction == "RIGHT_LEFT":
                    j = 0
                    for i in range(0, row):
                        seam_pts.append([j, i])
                        j = backtrack[i, j]
                elif former_direction == "DOWN_UP" and direction == "LEFT_RIGHT":
                    j = col - 1
                    for i in range(row - 1, -1, -1):
                        seam_pts.append([j, i])
                        j = backtrack[i, j]
                elif former_direction == "DOWN_UP" and direction == "RIGHT_LEFT":
                    j = 0
                    for i in range(row - 1, -1, -1):
                        seam_pts.append([j, i])
                        j = backtrack[i, j]
                else:
                    assert False, "No such possibility"
            else:
                ...
                for i in range(row - 1, -1, -1):
                    ...
                    j = backtrack[i, j]
                '''
                # 从两个方向来搜索缝合线，选出最居中的一条 （在@jit(nopython=True)下报错）
                temp_pts1 = []
                j = np.argmin(energy_map[-1, int(col / 3.0):int(2 * col / 3.0)])
                for i in range(row - 1, -1, -1):
                    temp_pts1.append([j, i])
                    j = backtrack[i, j]

                temp_pts2 = []
                j = np.argmin(energy_map[0, int(col / 3.0):int(2 * col / 3.0)])
                for i in range(0, row):
                    temp_pts2.append([j, i])
                    j = backtrack[i, j]

                temp_array1 = np.array(temp_pts1, dtype=np.float32)
                temp_array2 = np.array(temp_pts2, dtype=np.float32)
                if abs(np.mean(temp_array1[:, 0]) - col / 2.0) >= abs(np.mean(temp_array2[:, 0]) - col / 2.0):
                    seam_pts = temp_pts2
                else:
                    seam_pts = temp_pts1
                '''
        elif direction == "UP_DOWN" or direction == "DOWN_UP":
            if former_direction == "LEFT_RIGHT" or former_direction == "RIGHT_LEFT":
                if former_direction == "LEFT_RIGHT" and direction == "UP_DOWN":
                    i = row - 1
                    for j in range(0, col):
                        seam_pts.append([j, i])
                        i = backtrack[i, j]
                elif former_direction == "LEFT_RIGHT" and direction == "DOWN_UP":
                    i = 0
                    for j in range(0, col):
                        seam_pts.append([j, i])
                        i = backtrack[i, j]
                elif former_direction == "RIGHT_LEFT" and direction == "UP_DOWN":
                    i = row - 1
                    for j in range(col - 1, -1, -1):
                        seam_pts.append([j, i])
                        i = backtrack[i, j]
                elif former_direction == "RIGHT_LEFT" and direction == "DOWN_UP":
                    i = 0
                    for j in range(col - 1, -1, -1):
                        seam_pts.append([j, i])
                        i = backtrack[i, j]
                ...
                temp_pts2 = []
                i = np.argmin(energy_map[int(row / 3.0):int(2 * row / 3.0), 0])
                for j in range(0, col):
                    temp_pts2.append([j, i])
                    i = backtrack[i, j]
                ...
                temp_array1 = np.array(temp_pts1, dtype=np.float32)
                temp_array2 = np.array(temp_pts2, dtype=np.float32)
                if np.fabs(np.mean(temp_array1[:, 1]) - row / 2.0) >= np.fabs(np.mean(temp_array2[:, 1]) - row / 2.0):
                    seam_pts = temp_pts2
                else:
                    seam_pts = temp_pts1
                '''
    ...
    seam_pts = search_seam_pts(energy_map, backtrack, direction, former_direction, row, col)
    seam_pts = np.array(seam_pts, np.float32).reshape((-1, 2)) / shrink_scale
    seam_pts = seam_pts.astype(np.int32)
    return seam_pts


def getOverlayImg_Seam(img1, img2, former_direction, last_flag):
    img1_gray = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    _, mask1 = cv2.threshold(img1_gray, 0, 255, cv2.THRESH_BINARY)
    img2_gray = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    _, mask2 = cv2.threshold(img2_gray, 0, 255, cv2.THRESH_BINARY)
    ...
    for k in range(len(contours_overlay)):
        ...
        if abs(cX_dst - cX_overlay) >= abs(cY_dst - cY_overlay):
            if cX_dst >= cX_overlay:
                ...
                if former_direction == "RIGHT_LEFT":
                    print("Warning: 避免往回拼接，跳过本次图像拼接！")
                    return img1, former_direction
                # seam_pts = find_seam([img_for_energy[y:y + h, x:x + w]], direction, former_direction)  #计算能量图方法v1
                seam_pts = find_seam([img1[y:y + h, x:x + w], img2[y:y + h, x:x + w]], direction, former_direction)  # 计算能量图方法v2
                weight_array1 = get_seam_weight(seam_pts, (h, w), direction, mix_width)
                weight_array2 = 1.0 - weight_array1
            else:
                # print("重叠区从右向左拼接融合")
                direction = "RIGHT_LEFT"
                if former_direction == "LEFT_RIGHT":
                    print("Warning: 避免往回拼接，跳过本次图像拼接！")
                    return img1, former_direction
                # seam_pts = find_seam([img_for_energy[y:y + h, x:x + w]], direction, former_direction)  #计算能量图方法v1
                seam_pts = find_seam([img1[y:y + h, x:x + w], img2[y:y + h, x:x + w]], direction, former_direction)  # 计算能量图方法v2
...
def getMosiacImage(read_path, save_path="./", suffix=".jpg", isShowImg=False, isSaveImg=False):
    ...
    for img in img_generator(img_files, scale):
        if count > 0:
            ...
            # mergeImg = getOverlayImg_Distance(mergeImg, warpImg, last_flag)  # 按距离设置权重，过渡相对自然
            mergeImg, former_direction = getOverlayImg_Seam(mergeImg, warpImg, former_direction, last_flag)  # 最佳缝合线拼接
            ############################################################################
...
if __name__ == '__main__':
    # getReducedImages(read_path="./Original_Image_Set/",
    #                 save_path="./Reduce_Image_Set", suffix=".JPG")
    mergeImg = getMosiacImage(read_path="./Original_Image_Set/",
                              save_path="./Stitch_Image_Result/", suffix=".JPG", isShowImg=False, isSaveImg=True)
